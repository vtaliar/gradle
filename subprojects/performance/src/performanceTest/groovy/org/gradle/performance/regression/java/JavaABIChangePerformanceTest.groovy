...
class JavaABIChangePerformanceTest extends AbstractCrossVersionPerformanceTest {
    ...
    def "assemble for abi change on #testProject"() {
        ...
        def profiler = Profiler.create()
        def recordingsLocation = profiler.getJfrRecordingsLocation("compiler-daemon")
        if (recordingsLocation != null) {
            recordingsLocation.mkdirs()
            runner.args += ["-PjavaCompileJvmArgs=${profiler.getAdditionalJvmOpts(true, recordingsLocation).join(";")}"]
        }
        runner.warmUpRuns = 3
        runner.runs = 5
